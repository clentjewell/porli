...
test('public catalogue separates sector, availability and private drafts',async t=>{const{visitor,staff}=await fixture(t);const land=await visitor('/properties');assert.equal(land.data.properties.length,5);assert.ok(land.data.properties.every(p=>p.sector==='land'));assert.deepEqual(land.data.properties.map(p=>p.title).sort(),['Cedar Ridge','Fernwick Estate, Lot 12','Harbour Road Development Site','Orchard Lane Subdivision','Saltmere Valley Acreage']);const commercial=await visitor('/properties?sector=commercial');assert.equal(commercial.data.properties.length,3);assert.ok(commercial.data.properties.every(p=>p.sector==='commercial'));assert.ok(commercial.data.properties.some(p=>p.id==='grand-blue-hotel'));assert.ok(commercial.data.properties.some(p=>p.id==='demo-commercial-01'));assert.ok(commercial.data.properties.some(p=>p.id==='demo-commercial-02'));assert.equal((await visitor('/properties?sector=all')).data.properties.length,8);assert.equal((await visitor('/properties?sector=land&pending=1')).data.properties.length,6);assert.equal((await visitor('/properties/elmshore-heights-stage-2')).status,404);assert.equal((await visitor('/properties/courtyard-house')).status,404);assert.equal((await staff('/properties/elmshore-heights-stage-2')).status,200);assert.equal((await visitor('/properties?sector=residential')).status,400);assert.equal((await visitor('/properties?location=nowhere')).data.total,0);assert.equal((await visitor('/properties?min=-10')).status,400);assert.equal((await visitor('/admin/properties')).status,401);assert.equal((await visitor('/properties?sector=lettings')).status,400);});
test('saves are idempotent, persistent and scoped to current account',async t=>{const{customer,other,visitor}=await fixture(t);const body={property_id:'demo-land-01'};assert.equal((await visitor('/saves','POST',body)).status,401);await customer('/saves','POST',body);await customer('/saves','POST',body);assert.deepEqual((await customer('/saves')).data.ids,[body.property_id]);assert.deepEqual((await other('/saves')).data.ids,[]);await customer('/saves','DELETE',body);assert.deepEqual((await customer('/saves')).data.ids,[]);});
test('message retries reuse one thread and other accounts cannot read it',async t=>{const{customer,other,staff}=await fixture(t);const body={property_id:'demo-land-01',body:'Is there afternoon sun?',client_key:'retry-1'};const a=await customer('/conversations','POST',body),b=await customer('/conversations','POST',body);assert.equal(a.data.id,b.data.id);assert.equal((await customer('/conversations/'+a.data.id)).data.messages.length,1);assert.equal((await other('/conversations/'+a.data.id)).status,404);assert.equal((await other('/conversations/'+a.data.id+'/messages','POST',{body:'unauthorised',client_key:'hack'})).status,404);assert.equal((await staff('/conversations/'+a.data.id+'/messages','POST',{body:'Yes, it does.',client_key:'staff-1'})).status,200);assert.equal((await customer('/conversations/'+a.data.id)).data.messages.length,2);});
test('notes never reach customer responses and reading or noting is not replying',async t=>{const{customer,staff}=await fixture(t);const{id}= (await customer('/conversations','POST',{property_id:'demo-land-01',body:'Hello team',client_key:'m1'})).data;assert.equal((await staff('/admin/dashboard')).data.awaiting,1);await staff(`/conversations/${id}/read`,'POST',{});await staff(`/conversations/${id}/notes`,'POST',{body:'PRIVATE NOTE'});assert.equal((await staff('/admin/dashboard')).data.awaiting,1);const view=await customer('/conversations/'+id);assert.equal(view.data.notes,undefined);assert.ok(!JSON.stringify(view).includes('PRIVATE NOTE'));assert.equal((await customer(`/conversations/${id}/notes`,'POST',{body:'forged'})).status,403);await staff(`/conversations/${id}/messages`,'POST',{body:'A human reply',client_key:'reply1'});assert.equal((await staff('/admin/dashboard')).data.awaiting,0);});
test('metrics count people and property threads, never message volume',async t=>{const{customer,staff}=await fixture(t);for(let i=0;i<5;i++)await customer('/conversations','POST',{property_id:i<3?'demo-land-01':'demo-land-03',body:'Question '+i,client_key:'metric-'+i});const d=(await staff('/admin/dashboard')).data;assert.equal(d.people,1);assert.equal(d.enquired,2);assert.equal(d.new_conversations,2);});
test('inspection confirmation enforces capacity atomically and requests are not bookings',async t=>{const{customer,other,staff}=await fixture(t);const starts_at=new Date(Date.now()+5*86400000).toISOString();assert.equal((await staff('/admin/slots','POST',{property_id:'demo-land-01',starts_at,capacity:1})).status,201);const slot=(await customer('/properties/harbour-road-development-site')).data.slots.find(s=>s.capacity===1);const payload={property_id:'demo-land-01',slot_id:slot.id};const a=(await customer('/inspections','POST',payload)).data.id,b=(await other('/inspections','POST',payload)).data.id;assert.equal((await customer('/inspections','POST',payload)).data.id,a);assert.equal((await customer('/inspections')).data.inspections[0].status,'requested');assert.equal((await customer('/inspections/'+a,'PATCH',{status:'confirmed'})).status,403);const results=await Promise.all([staff('/inspections/'+a,'PATCH',{status:'confirmed'}),staff('/inspections/'+b,'PATCH',{status:'confirmed'})]);assert.deepEqual(results.map(r=>r.status).sort(),[200,409]);assert.equal((await other('/inspections/'+a,'PATCH',{status:'cancelled'})).status,404);});
test('closed enquiry does not sell property and new customer messages reopen it',async t=>{const{customer,staff}=await fixture(t);const{id}=(await customer('/conversations','POST',{property_id:'demo-land-01',body:'Hello',client_key:'open-1'})).data;await staff('/conversations/'+id,'PATCH',{stage:'closed'});assert.equal((await customer('/properties/harbour-road-development-site')).data.property.transaction_status,'available');await customer('/conversations/'+id+'/messages','POST',{body:'Another question',client_key:'open-2'});assert.equal((await staff('/conversations/'+id)).data.stage,'in_conversation');});
test('publication validation, revision conflicts and outcome reversals',async t=>{const{staff,visitor,db}=await fixture(t);let draft=(await staff('/properties/garden-cottage')).data.property;assert.equal((await staff('/admin/properties','PATCH',{...draft,publication_state:'published'})).status,400);let p=(await staff('/properties/harbour-road-development-site')).data.property;assert.equal((await staff('/admin/properties','PATCH',{...p,transaction_status:'leased'})).status,400);assert.equal((await staff('/admin/properties','PATCH',{...p,title:'Updated courtyard'})).status,200);assert.equal((await staff('/admin/properties','PATCH',{...p,title:'Stale overwrite'})).status,409);p=(await staff('/properties/harbour-road-development-site')).data.property;assert.equal((await staff('/admin/properties','PATCH',{...p,transaction_status:'sold'})).status,200);assert.equal((await staff('/admin/dashboard')).data.outcomes,1);assert.equal((await visitor('/properties')).data.total,4);p=(await staff('/properties/harbour-road-development-site')).data.property;await staff('/admin/properties','PATCH',{...p,transaction_status:'available'});assert.equal((await staff('/admin/dashboard')).data.outcomes,0);if(db)assert.equal(db.prepare('SELECT COUNT(*) n FROM audit').get().n,3);});
test('terminal properties block new enquiries and inspections but retain conversations',async t=>{const{customer,other,staff}=await fixture(t);const{id}=(await customer('/conversations','POST',{property_id:'demo-land-03',body:'Hello',client_key:'terminal1'})).data;let p=(await staff('/properties/fernwick-estate-lot-12')).data.property;await staff('/admin/properties','PATCH',{...p,transaction_status:'sold'});assert.equal((await other('/conversations','POST',{property_id:p.id,body:'new',client_key:'terminal2'})).status,409);assert.equal((await other('/inspections','POST',{property_id:p.id,proposed:'tomorrow'})).status,409);assert.equal((await customer('/conversations/'+id)).status,200);});
...
test('the Grand Blue Hotel headline listing is real, commercial and priced at A$9 million',async t=>{const{visitor}=await fixture(t);const commercial=await visitor('/properties?sector=commercial');const hotel=commercial.data.properties.find(p=>p.id==='grand-blue-hotel');assert.ok(hotel);assert.equal(hotel.is_demo,false);assert.equal(hotel.price_label,'');assert.equal(hotel.price_minor,900000000);assert.equal(hotel.currency,'AUD');assert.ok(!hotel.highlights.includes('Price to be confirmed'));assert.equal(hotel.sector,'commercial');assert.equal(hotel.sale_method,'expressions_of_interest');const asc=(await visitor('/properties?sector=commercial&sort=price-asc')).data.properties;assert.equal(asc.filter(p=>p.price_minor>0).at(-1).id,'grand-blue-hotel');const desc=(await visitor('/properties?sector=commercial&sort=price-desc')).data.properties;assert.equal(desc[0].id,'grand-blue-hotel');assert.ok((await visitor('/properties?sector=commercial&min=1')).data.properties.some(p=>p.id==='grand-blue-hotel'));});
test('publishing without a price requires a price label, and property type, sector and currency are validated',async t=>{const{staff}=await fixture(t);const media=[{url:'/assets/courtyard.webp',alt:'A'},{url:'/assets/apartment.webp',alt:'B'},{url:'/assets/townhouse.webp',alt:'C'}];const base={title:'Test Listing',sector:'commercial',property_type:'hotel',price_minor:0,sale_method:'expressions_of_interest',floor_area:0,land_area:0,zoning:'',tenancy:'',bedrooms:0,bathrooms:0,parking:0,locality:'Nowhere',summary:'Summary',description:'Description text.',publication_state:'published',transaction_status:'available',media,features:[]};assert.equal((await staff('/admin/properties','POST',{...base,price_label:'',sale_method:'private_sale'})).status,400);assert.equal((await staff('/admin/properties','POST',{...base,price_label:''})).status,200);const created=await staff('/admin/properties','POST',{...base,price_label:'Price to be confirmed'});assert.equal(created.status,200);assert.equal(created.data.property.price_label,'Price to be confirmed');assert.equal((await staff('/admin/properties','POST',{...base,price_label:'x',property_type:'castle'})).status,400);assert.equal((await staff('/admin/properties','POST',{...base,price_label:'x',currency:'EUR'})).status,400);assert.equal((await staff('/admin/properties','POST',{...base,price_label:'x',sector:'residential'})).status,400);});
...
test('property type must match the listing sector',async t=>{const{staff}=await fixture(t);const base={title:'Mismatch test',price_minor:0,price_label:'TBC',currency:'AUD',bedrooms:0,bathrooms:0,parking:0,locality:'Nowhere',summary:'S',description:'D',publication_state:'draft',transaction_status:'available',media:[],features:[],details:[],highlights:[],sale_method:'private_sale',floor_area:0,land_area:0,zoning:'',tenancy:''};assert.equal((await staff('/admin/properties','POST',{...base,sector:'land',property_type:'office'})).status,400);assert.equal((await staff('/admin/properties','POST',{...base,sector:'commercial',property_type:'development_site'})).status,400);assert.equal((await staff('/admin/properties','POST',{...base,sector:'land',property_type:'development_site'})).status,200);assert.equal((await staff('/admin/properties','POST',{...base,sector:'commercial',property_type:'office'})).status,200);});
...
test('why, feature groups and documents round-trip and reject malformed input',async t=>{const{staff}=await fixture(t);const p=(await staff('/properties/harbour-road-development-site')).data.property;
...
test('Featured is a named sort the visitor can turn off, not a hidden override',async t=>{const{visitor}=await fixture(t);
  ...
  assert.equal(asc.at(-1).slug,'millrace-warehouse','a price label still sorts last (docs/18)');
...
test('legacy rent rows migrate to buy on every start, idempotently',{skip:!!remote},async()=>{
  ...
  try{
    ...
    try{
      ...
      const res=await fetch(base+'/api/properties?sector=all');
...
test('land listings filter by type and size, and the residential demos are retired as archived',async t=>{const{visitor,staff}=await fixture(t);
  const rural=(await visitor('/properties?sector=land&type=rural')).data.properties;assert.deepEqual(rural.map(p=>p.title).sort(),['Cedar Ridge','Saltmere Valley Acreage']);
  const small=(await visitor('/properties?sector=land&size=-5000')).data.properties;assert.deepEqual(small.map(p=>p.id).sort(),['demo-land-01','demo-land-03']);
  const large=(await visitor('/properties?sector=land&size=100000-')).data.properties;assert.deepEqual(large.map(p=>p.id).sort(),['demo-land-05','demo-land-06']);
  const mid=(await visitor('/properties?sector=land&size=10000-100000')).data.properties;assert.deepEqual(mid.map(p=>p.id).sort(),['demo-land-07']);
  assert.equal((await visitor('/properties?sector=land&size=big')).status,400);assert.equal((await visitor('/properties?sector=land&size=-')).status,400);
  for(const p of [...rural,...small])assert.ok(p.land_area>0&&p.zoning&&p.sale_method);
  const admin=(await staff('/admin/properties')).data.properties;const homes=admin.filter(p=>p.sector==='residential');assert.ok(homes.length>0);assert.ok(homes.every(p=>p.publication_state==='archived'));
  assert.ok(!(await visitor('/properties?sector=all')).data.properties.some(p=>p.sector==='residential'));
  const edited=await staff('/admin/properties','PATCH',{...homes[0],version:homes[0].version,summary:'Still editable'});assert.equal(edited.status,200);assert.equal(edited.data.property.sector,'residential');});
test('a land listing publishes on a sale method alone, but a private sale still needs a price or a label',async t=>{const{staff}=await fixture(t);const media=[{url:'/assets/land-cedar-ridge-supplied.webp',alt:'A'},{url:'/assets/land-orchard-lane-supplied.webp',alt:'B'},{url:'/assets/land-mill-street.webp',alt:'C'}];const base={title:'Tender Lot',sector:'land',property_type:'development_site',price_minor:0,price_label:'',currency:'AUD',bedrooms:0,bathrooms:0,parking:0,locality:'Nowhere',summary:'S',description:'D',publication_state:'published',transaction_status:'available',media,features:[],details:[],highlights:[],floor_area:0,land_area:900,zoning:'Mixed use',tenancy:''};
...
test('aerial and site plan slots are validated, round-trip, and each fictional land listing ships with a site plan',async t=>{const{visitor,staff}=await fixture(t);
  const land=(await visitor('/properties?sector=land')).data.properties;assert.ok(land.length>0);for(const p of land){const plan=p.plans.find(x=>x.kind==='site_plan');assert.ok(plan);assert.match(plan.url,/^\/assets\/plan-[a-z0-9-]+\.webp$/);assert.match(plan.alt,/not a survey/);assert.match(p.media[0].alt,/(Openly licensed photograph|Reference photograph supplied by the team|Generated concept image)\.$/);if(/Generated concept image\.$/.test(p.media[0].alt))assert.match(p.media[0].caption,/render/i);assert.match(p.media[0].caption,/Wikimedia Commons|supplied by the RealDistrict team/);for(const a of p.plans.filter(x=>x.kind==='aerial'))assert.match(a.caption,/Wikimedia Commons|supplied by the RealDistrict team/);}assert.ok(land.some(p=>p.plans.some(x=>x.kind==='aerial')));
...
test('land sorts by size, largest first',async t=>{const{visitor}=await fixture(t);const rows=(await visitor('/properties?sector=land&sort=size-desc')).data.properties;assert.ok(rows.length>1);for(let i=1;i<rows.length;i++)assert.ok(rows[i-1].land_area>=rows[i].land_area);assert.equal(rows[0].slug,'cedar-ridge');});

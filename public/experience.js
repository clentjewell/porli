...
export function homeView({ properties, state, esc, icon, money, price, card, empty, statusBadges, typeLabel, landTypes = [], commercialTypes = [], options = () => '' }) {
  ...
  // The Browse tile fills whatever the last row of the tiled grid leaves (docs/63). Six columns on
  // desktop: three tiles, then two wider, then twos; four columns from 1000px, every tile a half.
  const n = gridHomes.length, used6 = n <= 3 ? 2 * n : n <= 5 ? 3 * (n - 3) : (2 * (n - 5)) % 6, used4 = (2 * n) % 4;
  const tileClass = `span6-${used6 ? 6 - used6 : 6} span4-${used4 ? 4 - used4 : 4}`;
  ...
  return `<div class="market-home">
    ...
    <section class="market-destinations market-width" data-reveal aria-label="Featured destinations">
      ...
      <div class="destination-grid" data-reveal-group>
        <a class="destination-card" data-reveal href="/properties?sector=land&location=Saltmere"><img src="/assets/dest-saltmere-supplied.webp" alt="A coastline with lagoons and open green land, a reference photograph of a real place elsewhere, supplied by the team, standing in for fictional Saltmere" loading="lazy" width="436" height="581"><span class="destination-scrim" aria-hidden="true"></span><span class="destination-body"><strong>Saltmere</strong><em>Coastal sites and acreage.</em><span class="destination-facts">${esc(profile('Saltmere','land'))}</span></span><span class="destination-arrow" aria-hidden="true">${icon('external')}</span></a>
    ...
    <section class="market-listings market-width" aria-labelledby="available-heading">
      ...
      <div class="market-property-grid showcase-grid" data-type-grid data-filter="all" data-reveal-group>${gridHomes.length ? gridHomes.map(p => `<div data-reveal data-type="${esc(p.property_type)}">${card(p, true)}</div>`).join('') + `<a class="home-grid-all ${tileClass}" data-reveal href="/properties?sector=land"><span class="eyebrow">Browse</span><strong>Every land and commercial listing</strong><span>Search by location, type, price and size.</span><span class="home-grid-all-arrow" aria-hidden="true">${icon('arrow')}</span></a>` : empty('No available properties right now.', 'Check back for new listings.', '', '')}</div>

...
export function homeView({ properties, state, esc, icon, money, price, card, empty, statusBadges, typeLabel }) {
  ...
  const heroInfo=featured?(real?`<div class="market-feature-info headline"><div class="eyebrow">Headline listing</div><span class="feature-price">${price(featured)}</span><h2>${esc(featured.title)}</h2><p>${esc(featured.locality)}</p>${(featured.highlights||[]).length?`<div class="chip-row">${featured.highlights.slice(0,3).map(h=>`<span class="chip">${esc(h)}</span>`).join('')}</div>`:''}<div class="feature-actions"><a class="btn" href="/properties/${esc(featured.slug)}">View the property ${icon('arrow')}</a><a class="btn secondary" href="/properties/${esc(featured.slug)}?enquire=1">Enquire</a></div></div>`:`<a class="market-feature-info" href="/properties/${esc(featured.slug)}"><div><span class="feature-price">${money(featured)}</span><h2>${esc(featured.title)}</h2><p>${featured.bedrooms} bedrooms <span>·</span> ${featured.bathrooms} bathrooms <span>·</span> ${featured.parking} car spaces</p></div><span class="feature-open" aria-label="View property">${icon('arrow')}</span></a>`):'';

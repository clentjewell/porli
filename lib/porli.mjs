// RealDistrict application core. Runtime-neutral: works on Node.js (server.mjs) and
...
// Two sectors: land and commercial (docs/59). Residential is retired; its key stays only so a
// listing created before the change still validates when a staff member edits it.
const SECTORS = ['land','commercial'];
const PROPERTY_TYPES = { land: ['development_site','commercial_industrial','rural','subdivision_investment'], commercial: ['office','retail','industrial','hotel','medical','showroom','other'], residential: ['house','apartment','townhouse','villa','land'] };
...
const SORTS = ['featured','newest','price-asc','price-desc','size-desc'];
...
// the price (A$9,000,000) was supplied by Clent on 23 September 2026 (docs/64); nothing here states a
// room count or an area, which are still to be confirmed.
const GRAND_BLUE_WHY = [
  ...
  'Room count, land and floor area and sale terms are to be confirmed with the RealDistrict team; the price is shown on the listing.'
].join('\n\n');
...
export function createPorli({ db, files, schema, fixture, demo = false, seed = demo, adminEmail = '', adminPassword = '', mapsEmbedKey = '' }) {
  ...
  for (const column of [
    ...
    "ALTER TABLE properties ADD COLUMN plans TEXT NOT NULL DEFAULT '[]'"
  ...
  run("UPDATE settings SET value='Land and commercial property, with one team to talk to.' WHERE key='intro' AND value='Residential and commercial property, with one team to talk to.'");
  ...
  // GrandBlue's price, supplied by Clent on 23 September 2026 (docs/64): A$9,000,000. Guarded by the
  // old "Price to be confirmed" label so it runs once and never overwrites a later price.
  { const gb = one("SELECT description,highlights,details FROM properties WHERE id='grand-blue-hotel' AND price_minor=0 AND price_label='Price to be confirmed'");
    if (gb) { const hl = JSON.parse(gb.highlights || '[]').filter(h => h !== 'Price to be confirmed'); const dt = JSON.parse(gb.details || '[]').filter(d => !(Array.isArray(d) && d[0] === 'Price'));
      run("UPDATE properties SET price_minor=900000000, price_label='', currency='AUD', highlights=?, details=?, description=? WHERE id='grand-blue-hotel'", JSON.stringify(hl), JSON.stringify(dt), String(gb.description).replace('Pricing, room count, land area and sale terms are to be confirmed','The price is A$9,000,000. Room count, land area and sale terms are to be confirmed')); } }
  run("UPDATE properties SET why=REPLACE(why,?,?) WHERE id='grand-blue-hotel'", 'Pricing, room count, land and floor area and sale terms are to be confirmed with the RealDistrict team.', 'Room count, land and floor area and sale terms are to be confirmed with the RealDistrict team; the price is shown on the listing.');
  const audit = (u, action, subject, details = {}) => run('INSERT INTO audit VALUES(?,?,?,?,?,?)', uid(), u.id, action, subject, JSON.stringify(details), now());
  const property = p => { if (!p) return null; const { mode, ...rest } = p; return { ...rest, media: JSON.parse(p.media), features: JSON.parse(p.features), details: JSON.parse(p.details), highlights: JSON.parse(p.highlights), feature_groups: JSON.parse(p.feature_groups || '[]'), documents: JSON.parse(p.documents || '[]'), plans: JSON.parse(p.plans || '[]'), is_demo: !!p.is_demo, address_display: p.address ? 'full' : 'locality_only', has_location: p.latitude !== 0 || p.longitude !== 0 }; };
  ...
  // Residential is replaced by Land (docs/59). The fictional homes are archived rather than
  // deleted, so conversations, saves and inspections that refer to them stay intact in the
  // workspace; nothing residential is offered publicly any more. Idempotent on every start,
  // and placed after the fixture seed so a fresh database ends up in the same shape as a
  // migrated one.
  run("UPDATE properties SET publication_state='archived' WHERE is_demo=1 AND sector='residential' AND publication_state<>'archived'");
  ...
  if (seed && !one("SELECT id FROM properties WHERE id='grand-blue-hotel'")) {
    ...
    const description = [
      ...
      'The price is A$9,000,000. Room count, land area and sale terms are to be confirmed with the RealDistrict team. The figures above are drawn from the property’s own published information.'
    ].join('\n\n');
    const media = [
    ...
    const highlights = ['Oceanfront on Mae Phim Beach','About two hours from Bangkok','Oversized pool and beachclub','Restaurant, bar and conference centre','Solar power and water treatment'];
    const details = [
    ...
    run(`INSERT OR IGNORE INTO properties(${hotelCols.join(',')}) VALUES(${hotelCols.map(()=>'?').join(',')})`,
      'grand-blue-hotel','grand-blue-hotel-thailand','GrandBlue Resort & Beachclub','buy','hotel',900000000,'','AUD',0,0,0,0,
  ...
  // Eight fictional land listings, one pair per land type (docs/59): kept in code like the
  // commercial pair, INSERT OR IGNORE so an existing database receives them on its next start.
  // Their images are generated concept studies (Higgsfield Soul Location, docs/60); two of the
  // four types share one image, as the fictional homes did.
  if (seed) {
    const landFixtures = [
      { id:'demo-land-01', slug:'harbour-road-development-site', plan:'plan-harbour-road', title:'Harbour Road Development Site', property_type:'development_site', locality:'Saltmere', land_area:2450, zoning:'Mixed use (fictional)', sale_method:'expressions_of_interest', price_minor:0, price_label:'', status:'available', state:'published', image:'land-harbour-road-supplied', aerial:'aerial-harbour-road-supplied',
        summary:'A level corner site with two frontages, a short walk from the harbour.',
        description:'Harbour Road Development Site is a 2,450 m² corner parcel with 48 metres of frontage to Harbour Road and a second frontage to the lane behind. The site is level and cleared, with water, sewer and power at the boundary and an existing crossover.\n\nMixed-use zoning allows shops or offices at street level with apartments above, subject to approval. Offered by expressions of interest. This is a fictional listing, illustrated with a reference photograph of a real place elsewhere, supplied by the RealDistrict team.',
        highlights:['Two street frontages','Mixed-use zoning','Services at the boundary'], details:[['Land area','2,450 m²'],['Frontage','48 m to Harbour Road'],['Zoning','Mixed use (fictional)'],['Services','Water, sewer and power at the boundary'],['Title','One lot, freehold']],
        features:['Level, cleared site','Existing crossover','Second frontage to the rear lane','Walking distance to the harbour'] },
      { id:'demo-land-02', slug:'mill-street-corner', plan:'plan-mill-street', title:'Mill Street Corner', property_type:'development_site', locality:'Fernwick', land_area:1180, zoning:'Residential growth (fictional)', sale_method:'auction', price_minor:0, price_label:'', status:'sold', state:'published', image:'land-mill-street', aerial:'',
        summary:'A compact corner block in the town centre, sold at auction.',
        description:'Mill Street Corner is an 1,180 m² corner block at the edge of the Fernwick town centre, zoned for medium-density housing. Sold at auction. This is a fictional listing, illustrated with openly licensed photographs of real places elsewhere.',
        highlights:['Town-centre corner','Medium-density zoning','Sold at auction'], details:[['Land area','1,180 m²'],['Frontage','32 m to Mill Street'],['Zoning','Residential growth (fictional)'],['Title','One lot, freehold']],
        features:['Corner block','Existing services','Level site'] },
      { id:'demo-land-03', slug:'fernwick-estate-lot-12', plan:'plan-fernwick-lot-12', title:'Fernwick Estate, Lot 12', property_type:'commercial_industrial', locality:'Fernwick', land_area:4800, zoning:'General industrial (fictional)', sale_method:'private_sale', price_minor:165000000, price_label:'', status:'available', state:'published', image:'land-fernwick-lot-12-supplied', aerial:'aerial-fernwick-lot-12-supplied',
        summary:'A serviced industrial lot on a sealed estate road, ready to build.',
        description:'Lot 12 is a 4,800 m² rectangular lot on Estate Drive within the Fernwick business estate. The lot is benched and compacted, with all services connected at the boundary and a sealed road frontage of 80 metres.\n\nGeneral industrial zoning suits warehousing, light manufacturing and depots, subject to approval. Offered for private sale. This is a fictional listing, illustrated with a render supplied by the RealDistrict team.',
        highlights:['Benched and serviced','80 m sealed frontage','Ready to build'], details:[['Land area','4,800 m²'],['Frontage','80 m to Estate Drive'],['Zoning','General industrial (fictional)'],['Services','All services at the boundary'],['Title','One lot, freehold']],
        features:['Rectangular, benched lot','Sealed estate road','B-double access to the estate','Estate covenants apply'] },
      { id:'demo-land-04', slug:'elmshore-gateway-lot-3', plan:'plan-elmshore-gateway', title:'Elmshore Gateway, Lot 3', property_type:'commercial_industrial', locality:'Elmshore', land_area:9200, zoning:'Light industrial (fictional)', sale_method:'expressions_of_interest', price_minor:0, price_label:'', status:'under_offer', state:'published', image:'land-elmshore-gateway', aerial:'aerial-elmshore-gateway',
        summary:'A large light-industrial lot with highway exposure, now under offer.',
        description:'Lot 3 at Elmshore Gateway is a 9,200 m² lot with 115 metres of frontage to Gateway Boulevard and exposure to the highway beyond. Light industrial zoning suits showrooms, trade supplies and logistics, subject to approval.\n\nOffered by expressions of interest and currently under offer. This is a fictional listing, illustrated with openly licensed photographs of real places elsewhere.',
        highlights:['Highway exposure','115 m frontage','Light industrial zoning'], details:[['Land area','9,200 m²'],['Frontage','115 m to Gateway Boulevard'],['Zoning','Light industrial (fictional)'],['Services','Power and water at the boundary; sewer nearby'],['Title','One lot, freehold']],
        features:['Highway exposure','Wide sealed frontage','Gently sloping site'] },
      { id:'demo-land-05', slug:'cedar-ridge', plan:'plan-cedar-ridge', title:'Cedar Ridge', property_type:'rural', locality:'Elmshore', land_area:420000, zoning:'Rural (fictional)', sale_method:'private_sale', price_minor:240000000, price_label:'', status:'available', state:'published', image:'land-cedar-ridge-supplied', aerial:'aerial-cedar-ridge-supplied',
        summary:'Forty-two hectares of grazing country with a ridge-top house site.',
        description:'Cedar Ridge is 42 hectares of undulating grazing land on Ridge Road, fenced into four paddocks with a dam in each. A cleared ridge-top site has views across the valley and power at the road. Two road frontages.\n\nRural zoning; a dwelling is permitted subject to approval. Offered for private sale. This is a fictional listing, illustrated with reference photographs of real places elsewhere, supplied by the RealDistrict team.',
        highlights:['42 hectares','Four fenced paddocks','Ridge-top house site'], details:[['Land area','42 ha'],['Frontage','640 m to Ridge Road'],['Zoning','Rural (fictional)'],['Water','Four dams; seasonal creek'],['Title','One lot, freehold']],
        features:['Fenced into four paddocks','Dam in each paddock','Power at the road','Cleared house site with views'] },
      { id:'demo-land-06', slug:'saltmere-valley-acreage', plan:'plan-saltmere-valley', title:'Saltmere Valley Acreage', property_type:'rural', locality:'Saltmere', land_area:165000, zoning:'Rural living (fictional)', sale_method:'price_on_application', price_minor:0, price_label:'', status:'available', state:'published', image:'land-saltmere-valley-supplied', aerial:'aerial-saltmere-valley-supplied',
        summary:'Sixteen and a half hectares of valley floor, ten minutes from the coast.',
        description:'Saltmere Valley Acreage is 16.5 hectares of mostly level valley floor on Valley Road, ten minutes by car from the coast at Saltmere. Established shelter belts, a seasonal creek and a fenced boundary. Rural-living zoning permits a dwelling subject to approval.\n\nPrice on application. This is a fictional listing, illustrated with a reference photograph of a real place elsewhere, supplied by the RealDistrict team.',
        highlights:['16.5 hectares','Ten minutes to the coast','Rural-living zoning'], details:[['Land area','16.5 ha'],['Frontage','410 m to Valley Road'],['Zoning','Rural living (fictional)'],['Water','Seasonal creek'],['Title','One lot, freehold']],
        features:['Level valley floor','Shelter belts','Fenced boundary','Power at the road'] },
      { id:'demo-land-07', slug:'orchard-lane-subdivision', plan:'plan-orchard-lane', title:'Orchard Lane Subdivision', property_type:'subdivision_investment', locality:'Fernwick', land_area:32000, zoning:'Residential growth (fictional)', sale_method:'tender', price_minor:0, price_label:'', status:'available', state:'published', image:'land-orchard-lane-supplied', aerial:'aerial-orchard-lane-supplied',
        summary:'An approved twelve-lot subdivision on the edge of town, offered by tender.',
        description:'Orchard Lane Subdivision is a 3.2 hectare parcel with approval for twelve residential lots of 1,800 to 3,200 m². Roads and services are designed and approved; construction has not started. Sold as one parcel with the approval in place.\n\nOffered by tender. This is a fictional listing, illustrated with a reference photograph of a real place elsewhere, supplied by the RealDistrict team.',
        highlights:['Approval for twelve lots','Designed services','One parcel, one approval'], details:[['Land area','3.2 ha'],['Approved lots','12, from 1,800 to 3,200 m²'],['Zoning','Residential growth (fictional)'],['Services','Designed and approved; not yet built'],['Title','One lot, freehold']],
        features:['Subdivision approval in place','Sealed road frontage','Gently sloping to the north','Town water and sewer available'] },
      { id:'demo-land-08', slug:'elmshore-heights-stage-2', plan:'plan-elmshore-heights', title:'Elmshore Heights, Stage 2', property_type:'subdivision_investment', locality:'Elmshore', land_area:56000, zoning:'Urban growth (fictional)', sale_method:'expressions_of_interest', price_minor:0, price_label:'', status:'available', state:'draft', image:'land-elmshore-heights', aerial:'aerial-elmshore-heights',
        summary:'The second stage of an established estate, fifteen lots with approval pending.',
        description:'Stage 2 of Elmshore Heights is a 5.6 hectare parcel beside the completed first stage, with a subdivision application for fifteen lots lodged and pending. Not yet published. This is a fictional listing, illustrated with openly licensed photographs of real places elsewhere.',
        highlights:['Fifteen lots proposed','Beside a completed stage','Approval pending'], details:[['Land area','5.6 ha'],['Proposed lots','15'],['Zoning','Urban growth (fictional)'],['Approval','Application lodged']],
        features:['Adjoins completed Stage 1','Services in the adjoining road'] }
    ];
    const landCols = ['id','slug','title','mode','sector','property_type','price_minor','price_label','sale_method','currency','is_demo','bedrooms','bathrooms','parking','locality','summary','description','publication_state','transaction_status','media','features','details','highlights','floor_area','land_area','zoning','tenancy','featured','created_at','listed_at','plans'];
    // The site-plan slot (docs/61) holds the illustrative plan drawn for each fictional parcel; the
    // aerial slot is left empty where no honest image exists, so the empty state is real.
    // Covers and aerials are openly licensed photographs of real places elsewhere (docs/61), never
    // the fictional parcel itself; the credit travels in the caption and the kind in the alt text.
    const LAND_CREDITS = {"land-elmshore-heights": {"credit": "Photo: Philip Mallis, CC BY-SA 2.0, Wikimedia Commons", "place": "Keilor East, Victoria"}, "aerial-saltmere-valley": {"credit": "Aerial: Bauple58, CC BY-SA 4.0, Wikimedia Commons", "place": "Barellan, New South Wales"}, "land-cedar-ridge": {"credit": "Photo: Thennicke, CC BY-SA 4.0, Wikimedia Commons", "place": "Holbrook, New South Wales"}, "land-saltmere-valley": {"credit": "Photo: Shiftchange, CC0, Wikimedia Commons", "place": "Hillview, Queensland"}, "land-mill-street": {"credit": "Photo: Shiftchange, CC0, Wikimedia Commons", "place": "Lamington, Queensland"}, "land-harbour-road": {"credit": "Photo: Shiftchange, CC0, Wikimedia Commons", "place": "Mundoolun, Queensland"}, "aerial-elmshore-heights": {"credit": "Aerial: Wikideas1, CC0, Wikimedia Commons", "place": "a subdivision under construction"}, "land-elmshore-gateway": {"credit": "Photo: Bob T, CC BY-SA 4.0, Wikimedia Commons", "place": "Williams Landing, Victoria"}, "land-orchard-lane-supplied": {"credit": "Reference photograph supplied by the RealDistrict team", "place": "a real housing estate elsewhere", "kind": "supplied"}, "aerial-orchard-lane-supplied": {"credit": "Reference photograph supplied by the RealDistrict team", "place": "a real housing estate elsewhere", "kind": "supplied"}, "land-saltmere-valley-supplied": {"credit": "Reference photograph supplied by the RealDistrict team", "place": "real farmland elsewhere", "kind": "supplied"}, "aerial-saltmere-valley-supplied": {"credit": "Reference photograph supplied by the RealDistrict team", "place": "real farmland elsewhere", "kind": "supplied"}, "land-cedar-ridge-supplied": {"credit": "Reference photograph supplied by the RealDistrict team", "place": "a real lakeside estate elsewhere", "kind": "supplied"}, "aerial-cedar-ridge-supplied": {"credit": "Reference photograph supplied by the RealDistrict team", "place": "a real suburban street elsewhere", "kind": "supplied"}, "land-fernwick-lot-12-supplied": {"credit": "Illustrative render supplied by the RealDistrict team", "place": "an imagined business park", "kind": "render"}, "aerial-fernwick-lot-12-supplied": {"credit": "Illustrative render supplied by the RealDistrict team", "place": "an imagined business park", "kind": "render"}, "land-harbour-road-supplied": {"credit": "Reference photograph supplied by the RealDistrict team", "place": "real land on the edge of a town elsewhere", "kind": "supplied"}, "aerial-harbour-road-supplied": {"credit": "Reference photograph supplied by the RealDistrict team", "place": "real land on the edge of a town elsewhere", "kind": "supplied"}, "aerial-elmshore-gateway": {"credit": "Aerial: Bob T, CC BY-SA 4.0, Wikimedia Commons", "place": "Williams Landing, Victoria"}};
    // A fourth kind (docs/62): a reference photograph the team supplied, of a real place elsewhere.
    const kindText = k => ({ supplied:'Reference photograph supplied by the team.', render:'Generated concept image.' })[LAND_CREDITS[k].kind] || 'Openly licensed photograph.';
    // A render is an image of an imagined place, not a photograph (docs/62).
    const landWhat = k => LAND_CREDITS[k].kind === 'render' ? `illustrative render of ${LAND_CREDITS[k].place}` : `photograph of a real place (${LAND_CREDITS[k].place})`;
    const landMedia = f => [{ url:`/assets/${f.image}.webp`, alt:`${f.title}: ${landWhat(f.image)}, used to illustrate this fictional listing. ${kindText(f.image)}`, caption:LAND_CREDITS[f.image].credit }];
    const landPlans = f => [{ kind:'site_plan', url:`/assets/${f.plan}.webp`, alt:`${f.title}: illustrative site plan of a fictional parcel, not a survey. Generated concept image.`, caption:'Illustrative site plan, not a survey' }, ...(f.aerial?[{ kind:'aerial', url:`/assets/${f.aerial}.webp`, alt:`Aerial ${landWhat(f.aerial)}, standing in for ${f.title}. ${kindText(f.aerial)}`, caption:LAND_CREDITS[f.aerial].credit }]:[]) ];
    for (const f of landFixtures) {
      if (!one('SELECT id FROM properties WHERE id=?', f.id)) {
        const createdAt = now();
        run(`INSERT OR IGNORE INTO properties(${landCols.join(',')}) VALUES(${landCols.map(()=>'?').join(',')})`,
          f.id, f.slug, f.title, 'buy', 'land', f.property_type, f.price_minor, f.price_label, f.sale_method, 'AUD', 1,
          0, 0, 0, f.locality, f.summary, f.description, f.state, f.status,
          JSON.stringify(landMedia(f)), JSON.stringify(f.features), JSON.stringify(f.details), JSON.stringify(f.highlights),
          0, f.land_area, f.zoning, '', 0, createdAt, createdAt, JSON.stringify(landPlans(f)));
        if (f.state === 'published' && f.status === 'available') for (let day = 2; day <= 4; day += 2) { const date = new Date(); date.setUTCDate(date.getUTCDate()+day); date.setUTCHours(1,0,0,0); run('INSERT INTO slots(id,property_id,starts_at,capacity) VALUES(?,?,?,?)',uid(),f.id,date.toISOString(),4); }
      }
    }
    // Land rows created before the plans column existed (docs/61): an untouched fictional row gets
    // its site plan; a row a staff member has edited (version > 1) is left alone.
    // Untouched fictional land rows (version 1: no staff member has saved them) follow the fixture's
    // current pictures, plans and description (docs/61, docs/62). Idempotent; an edited row is never
    // touched. Replaces the two narrower migrations of docs/61.
    for (const f of landFixtures) { const row = one('SELECT media,plans,description FROM properties WHERE id=? AND version=1', f.id); if (!row) continue; const media = JSON.stringify(landMedia(f)), plans = JSON.stringify(landPlans(f)); if (row.media !== media || row.plans !== plans || row.description !== f.description) run('UPDATE properties SET media=?, plans=?, description=? WHERE id=?', media, plans, f.description, f.id); }
  }
  // The land listings shipped for a few hours with illustrative site plans (docs/59); Clent asked
  // for photographs like the other listings, so the plans are replaced by generated studies
  // (docs/60). Guarded by the old alt text, so it runs once per row and never touches an edit.
  for (const row of all("SELECT id,media,description FROM properties WHERE id LIKE 'demo-land-%' AND media LIKE '%illustrative site plan%'")) {
    const image = { 'demo-land-01':'land-mill-street','demo-land-02':'land-mill-street','demo-land-03':'land-fernwick-lot-12','demo-land-04':'land-elmshore-gateway','demo-land-05':'land-cedar-ridge','demo-land-06':'land-cedar-ridge','demo-land-07':'land-orchard-lane','demo-land-08':'land-elmshore-heights' }[row.id];
    if (!image) continue;
    const title = one('SELECT title FROM properties WHERE id=?', row.id).title;
    run('UPDATE properties SET media=?, description=? WHERE id=?', JSON.stringify([{ url:`/assets/${image}.webp`, alt:`${title}: fictional land, generated concept study` }]), String(row.description).replace('This is a fictional listing with an illustrative site plan.','This is a fictional listing, illustrated with openly licensed photographs of real places elsewhere.'), row.id);
  }
  if (demo) for (const [id,name,role] of [['demo-customer','Alex Morgan','customer'],['demo-other','Jamie Ellis','customer'],['demo-staff','Sam Taylor','staff'],['demo-admin','Site administrator','admin']]) run('INSERT OR IGNORE INTO users VALUES(?,?,?,?,?,1)',id,name,`${id}@porli.test`,passwordHash(randomBytes(32).toString('hex')),role);
  ...
  for (const row of all('SELECT id,media,is_demo FROM properties')) {
    ...
    // A third kind since docs/61: an openly licensed photograph of a real place elsewhere, used to
    // illustrate a fictional listing. Its alt text already names the kind and is left alone.
    const next = media.map(m => {
      const alt = String(m?.alt || '');
      if (/openly licensed photograph\.|reference photograph supplied by the team\./i.test(alt)) return m;
  ...
  // Short captions for the photographs shown as plates on a feature page (docs/32). Keyed by image
  // so it is idempotent and never overwrites a caption the team has since edited. They describe what
  // the photograph shows, drawn from the same alternative text above, and assert nothing about the
  // property that the property has not published.
  const PHOTO_CAPTIONS = {
    '/assets/grand-blue-aerial.webp': 'The hotel and pool from the air',
    '/assets/grand-blue-pool.webp': 'The main pool',
    '/assets/grand-blue-beach.webp': 'Mae Phim Beach',
    '/assets/grand-blue-room.webp': 'A guest room',
    '/assets/grand-blue-sunset.webp': 'Sunset over the beach',
    '/assets/grand-blue-evening.webp': 'Evening dining by the pool',
  };
  for (const row of all('SELECT id,media FROM properties')) {
    let media; try { media = JSON.parse(row.media); } catch { continue; }
    if (!Array.isArray(media) || !media.length) continue;
    const next = media.map(m => (!m?.caption && PHOTO_CAPTIONS[m?.url]) ? { ...m, caption: PHOTO_CAPTIONS[m.url] } : m);
    const json = JSON.stringify(next);
    if (json !== row.media) run('UPDATE properties SET media=? WHERE id=?', json, row.id);
  }
  // The product is RealDistrict (docs/38). Text seeded under the old name is already stored in a
  // live database that is never rebuilt, so it is rewritten here on every start. Idempotent by
  // construction: once the old name is gone, every replacement is a no-op. Only prose columns are
  // touched — ids, slugs and image paths keep the names they were created with.
  for (const [table, columns] of [['properties', ['title','summary','description','why']], ['users', ['name']], ['settings', ['value']]]) {
    for (const column of columns) {
      try { run(`UPDATE ${table} SET ${column}=REPLACE(${column},'Porli','RealDistrict') WHERE ${column} LIKE '%Porli%'`); }
      catch { /* a column this build does not have */ }
    }
  }
  // Both spellings of the old bootstrap display name normalise to one that is not the brand.
  run("UPDATE users SET name='Site administrator' WHERE name IN ('Porli administrator','RealDistrict administrator')");
  run("INSERT OR IGNORE INTO settings VALUES('headline','Find your next place.')");
  run("INSERT OR IGNORE INTO settings VALUES('intro','Land and commercial property, with one team to talk to.')");
  ...
  if (adminEmail && adminPassword) {
    ...
    if (!existing) {
      ...
      else run('INSERT INTO users(id,name,email,password,role) VALUES(?,?,?,?,?)', uid(), 'Site administrator', email, passwordHash(adminPassword), 'admin');
  ...
  function requireStaff(u) { requireUser(u); if (!staff(u)) fail(403,'This area is for the RealDistrict team.'); }
  ...
  async function handle(request, { remoteAddress = '', secure = false, serveStatic } = {}) {
    ...
    try {
      ...
      // First-run setup (docs/52): while the site has no active team account at all, the first
      // administrator can be created from the team sign-in page. The moment one exists this
      // answers 409, so it is a door that opens once. An email that already belongs to a customer
      // is promoted only if the caller knows that account's password.
      const teamExists = () => !!one("SELECT id FROM users WHERE role IN ('staff','admin') AND active=1");
      if(path==='/api/setup' && method==='GET') return send(200,{needed:!teamExists()});
      if(path==='/api/setup' && method==='POST') { rateLimit(remoteAddress+':auth',15); if(teamExists()) fail(409,'The workspace already has a team account. Sign in instead.'); const name=text(body.name,80),email=text(body.email,200).toLowerCase(),password=text(body.password,256); if(!name || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)||password.length<12) fail(400,'Enter your name, a valid email and a password of at least 12 characters.'); let user=one('SELECT * FROM users WHERE email=?',email); if(user){ if(!passwordMatches(password,user.password)) fail(401,'An account with this email already exists; enter its password to make it the administrator.'); run("UPDATE users SET role='admin',active=1,name=? WHERE id=?",name,user.id); } else { const id=uid(); run("INSERT INTO users(id,name,email,password,role) VALUES(?,?,?,?,'admin')",id,name,email,passwordHash(password)); user=one('SELECT * FROM users WHERE id=?',id); } return send(201,login(one('SELECT * FROM users WHERE id=?',user.id))); }
      // Change password (docs/52): the current password is required, the new one follows the
      // registration rule, and every other session for the account ends.
      if(path==='/api/profile/password' && method==='PATCH') { requireUser(u); rateLimit(remoteAddress+':auth',15); const current=text(body.current,256),next=text(body.next,256); const row=one('SELECT password FROM users WHERE id=?',u.id); if(!passwordMatches(current,row.password)) fail(401,'Current password is incorrect.'); if(next.length<12) fail(400,'The new password needs at least 12 characters.'); if(next===current) fail(400,'Choose a different password.'); run('UPDATE users SET password=? WHERE id=?',passwordHash(next),u.id); run('DELETE FROM sessions WHERE user_id=? AND token<>?',u.id,hashToken(cookie)); return send(200,{ok:true}); }
      ...
      if(path==='/api/properties' && method==='GET') {
        ...
        else { const sector=q.get('sector')||'land';if(!['land','commercial','all'].includes(sector))fail(400,'Choose land or commercial.');rows=rows.filter(p=>(sector==='all'||p.sector===sector)&&(p.transaction_status==='available'||q.get('pending')==='1'&&p.transaction_status==='under_offer')); }
        ...
        // Land size is one filter, "min-max" in square metres with either end open ("10000-").
        let sizeMin=0,sizeMax=Infinity;if(q.get('size')){const m=/^(\d*)-(\d*)$/.exec(q.get('size'));if(!m||(!m[1]&&!m[2]))fail(400,'Land size must be a range in square metres.');sizeMin=Number(m[1]||0);sizeMax=m[2]?Number(m[2]):Infinity;}
        const search=(q.get('location')||'').trim().toLowerCase();const priceFiltered=q.get('min')||q.get('max');rows=rows.filter(p=>(!search||`${p.locality} ${p.title}`.toLowerCase().includes(search))&&(!q.get('type')||p.property_type===q.get('type'))&&(!priceFiltered||p.price_minor>0)&&(!q.get('min')||p.price_minor>=Number(q.get('min'))*100)&&(!q.get('max')||p.price_minor<=Number(q.get('max'))*100)&&(!q.has('beds')||(q.get('beds')==='0'?p.bedrooms===0:p.bedrooms>=Number(q.get('beds'))))&&(!q.get('baths')||p.bathrooms>=Number(q.get('baths')))&&(!q.get('tenancy')||p.tenancy===q.get('tenancy'))&&(!q.get('min_floor')||p.floor_area>=Number(q.get('min_floor')))&&(!q.get('size')||(p.land_area>=sizeMin&&p.land_area<=sizeMax)));
        ...
        else if(sort==='size-desc') rows.sort((a,b)=>b.land_area-a.land_area);
      ...
      if(path==='/api/admin/properties'&&['POST','PATCH'].includes(method)){requireStaff(u);const existing=body.id?one('SELECT * FROM properties WHERE id=?',body.id):null;if(body.id&&!existing)fail(404,'Listing not found.');if(existing&&Number(body.version)!==existing.version)fail(409,'Another team member changed this listing. Reload before saving.');const p={...(existing?property(existing):{id:uid(),slug:uid(),title:'Untitled listing',property_type:'development_site',price_minor:0,price_label:'',currency:'AUD',is_demo:1,bedrooms:0,bathrooms:0,parking:0,locality:'',summary:'',description:'',publication_state:'draft',transaction_status:'available',media:[],features:[],details:[],highlights:[],featured:0,available_date:'',listed_at:'',sector:'land',sale_method:'private_sale',floor_area:0,land_area:0,zoning:'',tenancy:'',address:'',latitude:0,longitude:0,directions:'',why:'',feature_groups:[],documents:[],plans:[]}),...body};
        ...
        // Rebuild each entry from known keys only, so an unexpected field cannot be stored and a
        // caption cannot grow past the space the plate gives it.
        p.media=p.media.map(m=>{const caption=text(m.caption,60);const entry={url:m.url,alt:text(m.alt,200)};if(caption)entry.caption=caption;return entry;});
        ...
        // Aerial view and site plan (docs/61): up to four images, each an uploaded or bundled image with
        // alternative text; rebuilt from known keys like the documents.
        if(!Array.isArray(p.plans)||p.plans.length>4)fail(400,'Each plan needs a kind, an uploaded image and descriptive alternative text.');
        for(const pl of p.plans) if(!pl||!['aerial','site_plan'].includes(pl.kind)||typeof pl.url!=='string'||!/^\/(assets|uploads)\/[a-zA-Z0-9_.-]+\.(webp|png|jpg)$/.test(pl.url)||!text(pl.alt,200)||!(await files.exists(pl.url)))fail(400,'Each plan needs a kind, an uploaded image and descriptive alternative text.');
        p.plans=p.plans.map(pl=>({kind:pl.kind,url:text(pl.url,200),alt:text(pl.alt,200),...(text(pl.caption||'',60)?{caption:text(pl.caption,60)}:{})}));
        ...
        if(p.publication_state==='published') {
          if(!text(p.title)||!text(p.locality)||!text(p.description)||(p.price_minor<=0&&!p.price_label&&p.sale_method==='private_sale')||!p.media.length)fail(400,'Published listings require a title, locality, a price, a price label or a sale method other than private sale, a description and a cover photograph.');
      ...
      // An administrator sets a new password for a team member or customer who has forgotten
      // theirs (docs/55). Not for their own account — that is the change-password form — and
      // every session of the account ends, so the person signs in fresh with what they were given.
      if(path==='/api/admin/team/password'&&method==='PATCH'){requireAdmin(u);if(body.id===u.id)fail(400,'Change your own password under your account.');const target=one('SELECT id FROM users WHERE id=?',text(body.id,64));if(!target)fail(404,'Account not found.');const password=text(body.password,256);if(password.length<12)fail(400,'The new password needs at least 12 characters.');run('UPDATE users SET password=? WHERE id=?',passwordHash(password),target.id);run('DELETE FROM sessions WHERE user_id=?',target.id);return send(200,{ok:true});}

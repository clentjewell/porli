...
export const securityHeaders = {
  ...
  'Content-Security-Policy': "default-src 'self'; img-src 'self' data:; script-src 'self'; style-src 'self'; connect-src 'self'; font-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'"
};
 ...
 */
export function createPorli({ db, files, schema, fixture, demo = false, seed = demo, adminEmail = '', adminPassword = '' }) {
  ...
  const property = p => p ? { ...p, media: JSON.parse(p.media), features: JSON.parse(p.features), details: JSON.parse(p.details), highlights: JSON.parse(p.highlights), rent_frequency: p.mode === 'rent' ? 'week' : null, is_demo: !!p.is_demo, address_display: 'locality_only' } : null;
  ...
  if (seed && !one("SELECT id FROM properties WHERE id='grand-blue-hotel'")) {
    ...
    const details = [
      ...
      ['Suvarnabhumi Airport','170 km, about 2 hours by car'],
      ['Bangkok','190 km, about 2.5 hours by car'],
    ...
    run('INSERT OR IGNORE INTO properties(id,slug,title,mode,property_type,price_minor,price_label,currency,is_demo,bedrooms,bathrooms,parking,locality,summary,description,publication_state,transaction_status,media,features,details,highlights,featured,created_at,listed_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
      ...
      'published','available',JSON.stringify(media),JSON.stringify(features),JSON.stringify(details),JSON.stringify(highlights),1,createdAt,createdAt);
  ...
  async function handle(request, { remoteAddress = '', secure = false, serveStatic } = {}) {
    ...
    try {
      ...
      if(path==='/api/session' && method==='GET') return send(200,{user:publicUser(u),demo});
      ...
      if(path==='/api/properties' && method==='GET') {
        ...
        const search=(q.get('location')||'').trim().toLowerCase();const priceFiltered=q.get('min')||q.get('max');rows=rows.filter(p=>(!search||`${p.locality} ${p.title}`.toLowerCase().includes(search))&&(!q.get('type')||p.property_type===q.get('type'))&&(!priceFiltered||p.price_minor>0)&&(!q.get('min')||p.price_minor>=Number(q.get('min'))*100)&&(!q.get('max')||p.price_minor<=Number(q.get('max'))*100)&&(!q.has('beds')||(q.get('beds')==='0'?p.bedrooms===0:p.bedrooms>=Number(q.get('beds'))))&&(!q.get('baths')||p.bathrooms>=Number(q.get('baths')))&&(!q.get('by')||!p.available_date||p.available_date<=q.get('by')));
        const priceRank=p=>p.price_minor===0?1:0;
        if(q.get('sort')==='price-asc') rows.sort((a,b)=>priceRank(a)-priceRank(b)||a.price_minor-b.price_minor);if(q.get('sort')==='price-desc')rows.sort((a,b)=>priceRank(a)-priceRank(b)||b.price_minor-a.price_minor);
      ...
      if(path==='/api/admin/properties'&&['POST','PATCH'].includes(method)){requireStaff(u);const existing=body.id?one('SELECT * FROM properties WHERE id=?',body.id):null;if(body.id&&!existing)fail(404,'Listing not found.');if(existing&&Number(body.version)!==existing.version)fail(409,'Another team member changed this listing. Reload before saving.');const p={...(existing?property(existing):{id:uid(),slug:uid(),title:'Untitled home',mode:'buy',property_type:'house',price_minor:0,price_label:'',currency:'AUD',is_demo:1,bedrooms:0,bathrooms:0,parking:0,locality:'',summary:'',description:'',publication_state:'draft',transaction_status:'available',media:[],features:[],details:[],highlights:[],featured:0,available_date:'',listed_at:''}),...body};
        if(!allowedStatus[p.mode]?.includes(p.transaction_status))fail(400,'Availability status does not match Buy / Rent.');if(!['house','apartment','townhouse','villa','land','hotel'].includes(p.property_type)||!['draft','published','archived'].includes(p.publication_state))fail(400,'Invalid property type or publication state.');for(const key of ['price_minor','bedrooms','bathrooms','parking'])if(!Number.isSafeInteger(Number(p[key]))||Number(p[key])<0)fail(400,'Price and property facts must be non-negative whole numbers.');

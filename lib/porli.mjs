...
export function createPorli({ db, files, schema, fixture, demo = false, seed = demo, adminEmail = '', adminPassword = '', mapsEmbedKey = '' }) {
  ...
  if (seed) {
    ...
    const LAND_CREDITS = {"land-elmshore-heights": {"credit": "Photo: Philip Mallis, CC BY-SA 2.0, Wikimedia Commons", "place": "Keilor East, Victoria"}, "aerial-saltmere-valley": {"credit": "Aerial: Bauple58, CC BY-SA 4.0, Wikimedia Commons", "place": "Barellan, New South Wales"}, "aerial-orchard-lane": {"credit": "Aerial: Graeme Bartlett, CC BY 3.0, Wikimedia Commons", "place": "Burrawang, New South Wales"}, "aerial-cedar-ridge": {"credit": "Aerial: Nick Pitsas, CSIRO, CC BY 3.0, Wikimedia Commons", "place": "the Murrumbidgee near Murrumbateman, NSW"}, "land-cedar-ridge": {"credit": "Photo: Thennicke, CC BY-SA 4.0, Wikimedia Commons", "place": "Holbrook, New South Wales"}, "land-saltmere-valley": {"credit": "Photo: Shiftchange, CC0, Wikimedia Commons", "place": "Hillview, Queensland"}, "land-mill-street": {"credit": "Photo: Shiftchange, CC0, Wikimedia Commons", "place": "Lamington, Queensland"}, "land-harbour-road": {"credit": "Photo: Shiftchange, CC0, Wikimedia Commons", "place": "Mundoolun, Queensland"}, "land-orchard-lane": {"credit": "Photo: Lenny K, CC BY 2.0, Wikimedia Commons", "place": "rural Queensland, from a balloon"}, "land-fernwick-lot-12": {"credit": "Photo: Shiftchange, CC0, Wikimedia Commons", "place": "Carole Park, Queensland"}, "aerial-elmshore-heights": {"credit": "Aerial: Wikideas1, CC0, Wikimedia Commons", "place": "a subdivision under construction"}, "land-elmshore-gateway": {"credit": "Photo: Bob T, CC BY-SA 4.0, Wikimedia Commons", "place": "Williams Landing, Victoria"}};
    const landMedia = f => [{ url:`/assets/${f.image}.webp`, alt:`${f.title}: openly licensed photograph of a real place (${LAND_CREDITS[f.image].place}), used to illustrate this fictional listing. Openly licensed photograph.`, caption:LAND_CREDITS[f.image].credit }];
    const landPlans = f => [{ kind:'site_plan', url:`/assets/${f.plan}.webp`, alt:`${f.title}: illustrative site plan of a fictional parcel, not a survey. Generated concept image.`, caption:'Illustrative site plan, not a survey' }, ...(f.aerial?[{ kind:'aerial', url:`/assets/${f.aerial}.webp`, alt:`Aerial photograph of a real place (${LAND_CREDITS[f.aerial].place}), openly licensed, standing in for ${f.title}. Openly licensed photograph.`, caption:LAND_CREDITS[f.aerial].credit }]:[]) ];
    ...
    for (const f of landFixtures) if (one("SELECT id FROM properties WHERE id=? AND version=1 AND plans='[]'", f.id)) run('UPDATE properties SET plans=? WHERE id=?', JSON.stringify(landPlans(f)), f.id);
    // The generated studies of docs/60 give way to openly licensed photographs (docs/61). Guarded by
    // the studies' alt text, so it runs once per untouched row and never over a staff member's edit.
    for (const f of landFixtures) { const row = one("SELECT description FROM properties WHERE id=? AND version=1 AND media LIKE '%generated concept study%'", f.id); if (row) run('UPDATE properties SET media=?, plans=?, description=? WHERE id=?', JSON.stringify(landMedia(f)), JSON.stringify(landPlans(f)), String(row.description).replace('This is a fictional listing with generated concept imagery.','This is a fictional listing, illustrated with openly licensed photographs of real places elsewhere.'), f.id); }
  }
  ...
  for (const row of all('SELECT id,media,is_demo FROM properties')) {
    ...
    const next = media.map(m => {
      ...
      if (alt.toLowerCase().includes('openly licensed photograph.')) return m;

...
const GRAND_BLUE_WHY = [
  ...
  'Pricing, room count, land and floor area and sale terms are to be confirmed with the RealDistrict team.'
].join('\n\n');
...
export function createPorli({ db, files, schema, fixture, demo = false, seed = demo, adminEmail = '', adminPassword = '', mapsEmbedKey = '' }) {
  ...
  if (seed && !one("SELECT id FROM properties WHERE id='grand-blue-hotel'")) {
    ...
    const description = [
      ...
      'Pricing, room count, land area and sale terms are to be confirmed with the RealDistrict team. The figures above are drawn from the property’s own published information.'
    ].join('\n\n');
    const media = [

...
export function createPorli({ db, files, schema, fixture, demo = false, seed = demo, adminEmail = '', adminPassword = '', mapsEmbedKey = '' }) {
  ...
  // Their images are illustrative site plans drawn in code, not aerial photographs.
  if (seed) {
    const landFixtures = [
      { id:'demo-land-01', slug:'harbour-road-development-site', title:'Harbour Road Development Site', property_type:'development_site', locality:'Saltmere', land_area:2450, zoning:'Mixed use (fictional)', sale_method:'expressions_of_interest', price_minor:0, price_label:'', status:'available', state:'published', image:'land-harbour-road',
        summary:'A level corner site with two frontages, a short walk from the harbour.',
        description:'Harbour Road Development Site is a 2,450 m² corner parcel with 48 metres of frontage to Harbour Road and a second frontage to the lane behind. The site is level and cleared, with water, sewer and power at the boundary and an existing crossover.\n\nMixed-use zoning allows shops or offices at street level with apartments above, subject to approval. Offered by expressions of interest. This is a fictional listing with an illustrative site plan.',
      ...
      { id:'demo-land-02', slug:'mill-street-corner', title:'Mill Street Corner', property_type:'development_site', locality:'Fernwick', land_area:1180, zoning:'Residential growth (fictional)', sale_method:'auction', price_minor:0, price_label:'', status:'sold', state:'published', image:'land-mill-street',
        ...
        description:'Mill Street Corner is an 1,180 m² corner block at the edge of the Fernwick town centre, zoned for medium-density housing. Sold at auction. This is a fictional listing with an illustrative site plan.',
      ...
      { id:'demo-land-06', slug:'saltmere-valley-acreage', title:'Saltmere Valley Acreage', property_type:'rural', locality:'Saltmere', land_area:165000, zoning:'Rural living (fictional)', sale_method:'price_on_application', price_minor:0, price_label:'', status:'available', state:'published', image:'land-saltmere-valley',
        summary:'Sixteen and a half hectares of valley floor, ten minutes from the coast.',
        description:'Saltmere Valley Acreage is 16.5 hectares of mostly level valley floor on Valley Road, ten minutes by car from the coast at Saltmere. Established shelter belts, a seasonal creek and a fenced boundary. Rural-living zoning permits a dwelling subject to approval.\n\nPrice on application. This is a fictional listing with an illustrative site plan.',
      ...
      { id:'demo-land-08', slug:'elmshore-heights-stage-2', title:'Elmshore Heights, Stage 2', property_type:'subdivision_investment', locality:'Elmshore', land_area:56000, zoning:'Urban growth (fictional)', sale_method:'expressions_of_interest', price_minor:0, price_label:'', status:'available', state:'draft', image:'land-elmshore-heights',
        ...
        description:'Stage 2 of Elmshore Heights is a 5.6 hectare parcel beside the completed first stage, with a subdivision application for fifteen lots lodged and pending. Not yet published. This is a fictional listing with an illustrative site plan.',
    ...
    for (const f of landFixtures) {
      if (!one('SELECT id FROM properties WHERE id=?', f.id)) {
        ...
        run(`INSERT OR IGNORE INTO properties(${landCols.join(',')}) VALUES(${landCols.map(()=>'?').join(',')})`,
          ...
          JSON.stringify([{ url:`/assets/${f.image}.webp`, alt:`${f.title}: illustrative site plan of a fictional parcel, not a survey` }]), JSON.stringify(f.features), JSON.stringify(f.details), JSON.stringify(f.highlights),

  ...
  async function handle(request, { remoteAddress = '', secure = false, serveStatic } = {}) {
    ...
    try {
      ...
      if(path==='/api/properties' && method==='GET') {
        ...
        // A featured real listing leads the default view: it is the only stock that can actually be
        // sold, and newest-first buried it. An explicit price sort is left strictly alone, so the
        // price-label rule in docs/18 still holds and a chosen sort is never quietly overridden.
        else rows=[...rows.filter(p=>p.featured&&p.is_demo===false),...rows.filter(p=>!(p.featured&&p.is_demo===false))];
